...
async function main() {
	...
	try {
		...
		client.once('ready', async () => {
			...
			const abusedServer = await client.guilds.fetch(process.env.SERVER_ID);
			const result = await abusedServer.members.fetch(process.env.DISCORD_ID)
			let status;
...
main()
client.login(process.env.BOT_TOKEN).catch(function(error) {
	throw "The Bot Token Invalid"
})
